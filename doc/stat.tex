...
\end{enumerate}
performs well in a wide range of situations. The second step can be performed using the step method \texttt{FieldStepper}, which is provided by \texttt{generic-mbg}. Section \ref{sub:mcmc-init} describes how to use \texttt{FieldStepper}. 
...
\subsubsection{Postprocessing for areal prediction}  
...
\begin{eqnarray*}
    h\left(\int_{a_1\times\tau_1} g_1(f_1,\ldots,f_n,x,t)da dt, \ldots,\int_{a_m\times\tau_m} g_m(f_1,\ldots,f_n,x,t)da dt\right)
\end{eqnarray*}
where $a_1,\ldots,a_m$ are regions and $da$ denotes integration over area; $\tau_1\ldots\tau_m$ are time intervals and $dt$ denotes integration over time; $x$ is a geographical location; $t$ is a particular time; $f_1,\ldots,f_n$ are the Gaussian random fields in the model; and $g_1\ldots g_m$ and $h$ are arbitrary functions. This general specification covers many `volumetric' estimates of epidemiological interest. Three concrete examples are given below.

\begin{description}
    \item[\emph{Plasmodium falciparum} clinical burden in a country.] In this case, $n=m=1$. $f_1$ is the Gaussian random field, with nugget incorporated, corresponding to parasite rate. $g_1$ transforms this value to parasite rate (with a random age-correction factor), and thence to clinical incidence, and multiplies it by the population density of the pixel containing $x$. $a$ is the administrative unit under consideration and $\tau$ is most likely a particular year. $h$ multiplies its argument by the total area $|a|$ of $a$.
      ...
      "geometries": [
        { "type": "MultiPolygon",
          "coordinates": [
            ...
            "properties": {"name": "r1", "tmin": 2000.0, "tmax": 2007.5}
          }
        { "type": "MultiPolygon",
          "coordinates": [
            [[[102.0, 2.0], [103.0, 2.0], [103.0, 3.0], [102.0, 3.0], [102.0, 2.0]]],
            ...
            "properties": {"name": "r2", "tmin": 1993.5, "tmax": 2000.0}
          }
      ]
      "properties": {"name": "gc1"}
    }
...
The specializing module must expose an attribute called \texttt{areal\_postproc}, which like \texttt{map\_postproc} and \texttt{validate\_postproc} must be a list of functions. Each must take a Shapely geometry collection as its lone argument and return two values: \texttt{h}, which must be of appropriate arity to fulfil its role given above; and \texttt{g}, which must be a dictionary of functions keyed by area name. Each value in \texttt{g} mist take as arguments the random fields in the model, an array of locations (lat/lon or lat/lon/t, depending on whether the model is temporal) and optionally any other variables in the model. The following specifications might implement concrete examples above:
\begin{verbatim}
    def prevalence(gc):
    ...
    def diff_prevalence(gc):
        "Total change in Plasmodium falciparum clinical burden in a country between two consecutive years"
    ...
    def hbs_diff(gc):
        ...
        def h(*l):
            return np.diff(l)
        g = dict([geom.name, lambda S, a=geom.area: generic_mbg.invlogit(S)/a] for geom in gc.geoms)
        return h, g
\end{verbatim}
\subsection{Initializing the MCMC object}
\label{sub:mcmc-init} 

The shell command \texttt{mbg-infer} will call \texttt{make\_model} and wrap the jumble of variables it returns in a PyMC \texttt{MCMC} object. Before it begins sampling, it will call \texttt{mcmc\_init} on the MCMC object. This is your chance to assign custom step methods, etc. For example, to use \texttt{generic-mbg}'s \texttt{FieldStepper} on the Gaussian field evaluation in section \ref{sub:example}:
\begin{verbatim}
    def mcmc_init(M):
